...
import base.state.expPages.BijectionExp;
import base.state.menu.PuzMenu;
import java.awt.Graphics;
import java.awt.Color;
import java.util.Random;

public class BijectivePuz extends Puzzle {
	
	private int level;
	private int ctrlY, numSinks, numSources;
	
	public BijectivePuz(int w, int h) {
		super(w,h,10);
	...
	public BijectivePuz(int w, int h, int lev) {
		super(w,h,10);
	...
	public State whichState(int x, int y) {
		if (1050 < x && x < 1150 && 450 < y && y < 480 && solved && level == 5) {
			State s = new PuzMenu(getWidth(),getHeight(),"You have solved the \"bijective functions\" puzzle! Please continue!");
			return s;
		}
		if (1050 < x && x < 1150 && 450 < y && y < 480 && solved && level != 5) {
			State s = new BijectivePuz(getWidth(),getHeight(),level);
			return s;
		}
		...
		if (1050 < x && x < 1150 && 490 < y && y < 520) {
			State s = new BijectivePuz(getWidth(), getHeight());
			return s;
		}
		return this;
	}
	...
	public void write (Graphics g) {
		...
		g.setColor(Color.WHITE);
		g.drawString("Your goal is to create a bijective function.",110,120);
		
		g.drawString("Your may click elements of the domain and",110,160);
		g.drawString("map them to elements of the range.",110,180);
		
		g.drawString("A bijective function is a function is both",110,220);
		g.drawString("injective and surjective.",110,240);
