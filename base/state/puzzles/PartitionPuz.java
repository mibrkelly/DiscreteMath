...
public class PartitionPuz extends State {
	...
	public PartitionPuz(int w, int h) {
		super(w,h);
		...
		pieces[0] = new ProofPiece(130,120,0xff0000);
		pieces[1] = new ProofPiece(170,120,0x0000ff);
	...
	public PartitionPuz(int w, int h, int lev) {
		super(w,h);
		level = lev;
	...
	public State whichState(int x, int y) {
		if (1050 < x && x < 1150 && 450 < y && y < 480 && solved) {
			...
			State s= new PuzMenu(getWidth(),getHeight(),"You have solved the \"partition\" puzzle! Please continue!");
			return s;
		}
	...
	public void render(int keepTime) {
		...
		if (level == 2 || level == 3) {
			Square(495,50,10,550,0xffff00);
		}
		if (level == 1 || level == 4) {
			Square(350,50,10,550,0xffff00);
			Square(650,50,10,550,0xffff00);
		}
