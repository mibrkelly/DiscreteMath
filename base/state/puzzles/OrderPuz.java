/*Change this so there are 3 puzzles, <, >= and divides (as a partial ordering).*/

package base.state.puzzles;

import base.state.*;
import base.state.puzzles.pieces.*;
import base.state.expPages.OrderExp;
import base.state.menu.PuzMenu;
import java.awt.Graphics;
import java.awt.Color;

public class OrderPuz extends State {
	...
	public OrderPuz(int w, int h) {
		...
		pieces[3] = new NumberSource(960,100,0);
	...
	public OrderPuz(int w, int h, int lev) {
		super(w,h);
		...
		if (level == 2) {
			...
			pieces[12] = new PairPiece(100,500);
			pieces[13] = new PairPiece(100,550);
		}
		...
		if (level == 3) {
			...
			pieces[4] = new NumberSource(680,150,4);
			pieces[5] = new NumberSource(760,150,5);
			pieces[6] = new NumberSource(840,150,6);
		
			pieces[7] = new PairPiece(100,100);
			
			pieces[8] = new PairPiece(100,150);
			pieces[9] = new PairPiece(100,200);
			pieces[10] = new PairPiece(100,250);
			pieces[11] = new PairPiece(100,300);
			pieces[12] = new PairPiece(100,350);
			pieces[13] = new PairPiece(100,400);
			pieces[14] = new PairPiece(100,450);
			
			pieces[15] = new PairPiece(100,500);
			pieces[16] = new PairPiece(100,550);
			pieces[17] = new PairPiece(200,100);
			pieces[18] = new PairPiece(200,150);
			
			pieces[19] = new PairPiece(200,200);
			pieces[20] = new PairPiece(200,250);
			pieces[21] = new PairPiece(200,300);
			
			pieces[22] = new PairPiece(200,350);
			pieces[23] = new PairPiece(200,400);
			
			pieces[24] = new PairPiece(200,450);
			pieces[25] = new PairPiece(200,500);
			
			pieces[26] = new PairPiece(200,550);
			pieces[27] = new PairPiece(300,100);
		}
	...
	public State whichState(int x, int y) {
		if (1050 < x && x < 1150 && 450 < y && y < 480 && solved) {
			level++;
			if (level < 4) {
				State s= new OrderPuz(getWidth(),getHeight(),level);
				return s;
			}
			State s= new PuzMenu(getWidth(),getHeight(),"You have solved the \"order relation\" puzzle! Please continue!");
			return s;
		}
	...
	public void runCheck() {
		...
		if (level == 1) {
			for (int i = 4; i < pieces.length; i++) {
				if (((PairPiece)pieces[i]).getLeft() == 0 && ((PairPiece)pieces[i]).getRight() == 1) {
					goOn = true;
				}
			}
			if (!goOn)
			...
			goOn = false;
			for (int i = 4; i < pieces.length; i++) {
				if (((PairPiece)pieces[i]).getLeft() == 0 && ((PairPiece)pieces[i]).getRight() == 3) {
					goOn = true;
				}
			}
			if (!goOn)
			...
			goOn = false;
			for (int i = 4; i < pieces.length; i++) {
				if (((PairPiece)pieces[i]).getLeft() == 1 && ((PairPiece)pieces[i]).getRight() == 2) {
					goOn = true;
				}
			}
			if (!goOn)
		...
		if (level == 2) {
			...
			goOn = false;
			for (int i = 4; i < pieces.length; i++) {
				if (((PairPiece)pieces[i]).getLeft() == 1 && ((PairPiece)pieces[i]).getRight() == 0) {
					goOn = true;
				}
			}
			if (!goOn)
			...
			goOn = false;
			for (int i = 4; i < pieces.length; i++) {
				if (((PairPiece)pieces[i]).getLeft() == 3 && ((PairPiece)pieces[i]).getRight() == 0) {
					goOn = true;
				}
			}
			if (!goOn)
			...
			goOn = false;
			for (int i = 4; i < pieces.length; i++) {
				if (((PairPiece)pieces[i]).getLeft() == 3 && ((PairPiece)pieces[i]).getRight() == 2) {
					goOn = true;
				}
			}
			if (!goOn)
		...
		if (level == 3) {
			...
			goOn = false;
			for (int i = 7; i < pieces.length; i++) {
				if (((PairPiece)pieces[i]).getLeft() == 1 && ((PairPiece)pieces[i]).getRight() == 5) {
					goOn = true;
				}
			}
			if (!goOn)
			...
			goOn = false;
			for (int i = 7; i < pieces.length; i++) {
				if (((PairPiece)pieces[i]).getLeft() == 1 && ((PairPiece)pieces[i]).getRight() == 6) {
					goOn = true;
				}
			}
			if (!goOn)
			...
			goOn = false;
			for (int i = 7; i < pieces.length; i++) {
				if (((PairPiece)pieces[i]).getLeft() == 2 && ((PairPiece)pieces[i]).getRight() == 6) {
					goOn = true;
				}
			}
			if (!goOn)
			...
			goOn = false;
			for (int i = 7; i < pieces.length; i++) {
				if (((PairPiece)pieces[i]).getLeft() == 3 && ((PairPiece)pieces[i]).getRight() == 6) {
					goOn = true;
				}
			}
			if (!goOn)
			...
			goOn = false;
			for (int i = 7; i < pieces.length; i++) {
				if (((PairPiece)pieces[i]).getLeft() == 4 && ((PairPiece)pieces[i]).getRight() == 4) {
					goOn = true;
				}
			}
			if (!goOn)
	...
	public void write(Graphics g) {
		...
		if (level == 1) {
			g.drawString("Your goal is to create the order relation \"less than\" on the set. You are on level 1",120,620);
		}
		else if (level == 2) {
			g.drawString("Your goal is to create the order relation \"greater than or equal to\" on the set. You are on level 2",120,620);
		}
		else {
			g.drawString("Your goal is to create the partial order relation \"divides\" on the set, as in a divides b. You are on level 3",120,620);
		}
