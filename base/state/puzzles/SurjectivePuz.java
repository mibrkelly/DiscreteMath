...
import base.state.expPages.SurjectiveExp;
import base.state.menu.PuzMenu;
import java.awt.Graphics;
import java.awt.Color;
import java.util.Random;

public class SurjectivePuz extends Puzzle {
	
	private int level;
	private int ctrlY, numSinks, numSources;
	
	public SurjectivePuz(int w, int h) {
		super(w,h,10);
	...
	public SurjectivePuz(int w, int h, int lev) {
		super(w,h,10);
	...
	public State whichState(int x, int y) {
		if (1050 < x && x < 1150 && 450 < y && y < 480 && solved && level == 5) {
			State s = new PuzMenu(getWidth(),getHeight(),"You have solved the \"surjective functions\" puzzle! Please continue!");
			return s;
		}
		if (1050 < x && x < 1150 && 450 < y && y < 480 && solved && level != 5) {
			State s = new SurjectivePuz(getWidth(),getHeight(),level);
			return s;
		}
		...
		if (1050 < x && x < 1150 && 490 < y && y < 520) {
			State s = new SurjectivePuz(getWidth(), getHeight());
			return s;
		}
		return this;
	}
	...
	public void write (Graphics g) {
		...
		g.drawString("Your goal is to create a surjective function.",110,120);
		
		g.drawString("Your may click elements of the domain and",110,160);
		g.drawString("map them to elements of the range.",110,180);
		
		g.drawString("A surjective function is a function which",110,220);
		g.drawString("maps to everything in the domain.",110,240);
